...
| `node_exclude` | 空（不过滤） | 按节点名关键字排除订阅节点：节点名**包含任一关键字**即从节点池剔除（区分大小写）。**留空/缺省时不过滤，全部订阅节点（含美国/英国等欧美节点）均可选择**；仅当某地区段被 DeepSeek 风控拉黑时再按需填入，等效“在机场侧下掉”风险节点。新增/刷新订阅时自动过滤，启动加载旧缓存时同样生效，无需手工清洗 `mihomo_subscriptions.json`。示例：`["🇺🇸", "🇬🇧"]`。注意：已部署实例的该字段在各自 `config.json` 中，示例配置变更不影响存量部署，需手工调整 |
