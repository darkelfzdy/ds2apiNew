...
| `node_exclude` | 空（不过滤） | 按节点名关键字排除订阅节点：节点名**包含任一关键字**即从节点池剔除（区分大小写）。**留空/缺省时不过滤，全部订阅节点（含美国/英国等欧美节点）均可选择**；仅当某地区段被 DeepSeek 风控拉黑时再按需填入，等效“在机场侧下掉”风险节点。可在 WebUI「代理桥」页直接编辑，保存后立即生效并按新列表重滤节点池；新增/刷新订阅、启动加载旧缓存时同样过滤，无需手工清洗 `mihomo_subscriptions.json`。此前被过滤掉的节点在放宽关键字后需刷新订阅才会恢复。示例：`["🇺🇸", "🇬🇧"]` |
